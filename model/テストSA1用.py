...
import random
import math
from moussaif import fa_sen,fa_dasukai2, hulistics_1, hulistics_1_kai,cal_fij, fa_kekka , dasu_v_1d, fa_car , alpha_dasu,mokutekiti_siro, alpha_kousin, syouhan
...
ppp = Path("yomitori")
df = pd.read_excel('yomitori/iiyatu_test12.xlsx', sheet_name='Sheet4')
nmp=df.to_numpy()
syudan = nmp
...
IDs = np.array(syudan_gen[:,1])
for i in range(n_gen):
    print(IDs[i])

syoki_time= np.array(syudan_gen[:,0])
v_1d = syudan_gen[:,6]
v_2d = syudan_gen[:, 4:6]
alpha = syudan_gen[:, 9]
mokuteki = syudan_gen[:,7:9]
kiseki = np.empty((0,6))
for i in range(n_gen):
    tasu = np.array([[syoki_time[i],IDs[i], xy_iti[i,0],xy_iti[i,1],v_2d[i,0],v_2d[i,1]]])
    kiseki = np.append(kiseki,tasu,axis=0)
...
for j in zikan:
    ...
    iti_x = []
    iti_y = []
    hu_car = np.empty(n_gen)
    print(alpha,"alphaまえまえ")
    for f in range(5):
        for a in range(50):
            #周囲のエージェントを認識
            fa_syucar = fa_sen(alpha, xy_iti , n_gen, cars, n_cars)
            for i in range(n_gen):
                if fa_syucar[i] <10:
                    print("fa_syucar[i",i,fa_syucar[i], "は小さい",xy_iti[i],alpha[i])

            fa = fa_dasukai2(v_2d, xy_iti, n_gen)
            fa = fa_kekka(n_gen, fa, fa_syucar,hu_car)
            #print(fa, type(fa))
            #print("fa", fa)
            #alpha = alpha_ini
            #print("目的地",mokuteki)
            alpha_kousin(mokuteki, alpha,xy_iti,n_gen)
            alpha = hulistics_1(fa, alpha, n_gen,hu_car)
        ...
        for i in range(n_gen):
            ...
            dvdt[i, 0] = round(dvdt[i,0], 2)/10

            dvdt[i, 1] = (v_1d[i] * math.sin(alpha[i]) - v_2d[i, 1] ) / 0.5 + sum_fij[i, 1]
            dvdt[i, 1] = round(dvdt[i, 1], 2)/10

        v_2d += dvdt
        v_1d = dasu_v_1d(v_2d, v_1d, n_gen)
    print(alpha,"alpha")
    time = np.full(n_gen,j )
    xy_iti = xy_iti + v_2d
    for i in range(n_gen):
        tasu = np.array([[j,IDs[i],xy_iti[i,0],xy_iti[i,1],v_2d[i,0],v_2d[i,1]]])
        kiseki = np.append(kiseki,tasu,axis=0)
...
aaa = Path("hozon")
anim.save(aaa/'テスト回帰分析SA1.gif', writer='writer', fps=2)

df1 = pd.DataFrame(kiseki)
df1 = df1.rename(columns={0: 'time', 1: 'ID', 2:'X',3:'Y',4:'Vx',5:'Vy'})
df1[['ID']].astype('int')

plt.show()


df1.to_excel('テスト_軌跡SA1.xlsx')