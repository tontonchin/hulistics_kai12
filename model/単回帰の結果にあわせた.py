...
df = pd.read_excel("MAH00556-01_tankaiki_kekka.xlsx", sheet_name='Sheet1')
nmp=df.to_numpy()
syudan = nmp
...
for j in range(n_gen):
    ...
    moku_j = np.array([[syudan_gen[:,7], syudan_gen[:,8]]])
...
aaa = Path("hozon")
anim.save(aaa/'zikkou_tankaiki.gif', writer='writer', fps=2)
