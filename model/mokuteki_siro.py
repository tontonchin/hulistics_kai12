"""このプログラムは実行部です"""
#目的地に対して希望角度を逐次的に変化させる

import numpy as np
import random
import math
from moussaif import fa_dasukai,fa_dasukai2, hulistics_1, cal_fij, cal_fiw , dasu_v_1d, dvdt , alpha_dasu,mokutekiti_siro, alpha_kousin
...
df = pd.read_excel("MAH00556-01_nantoka_kai.xlsx", sheet_name='Sheet3')
nmp=df.to_numpy()
syudan = nmp
...
for j in zikan:
    ...
    if not j == 0:
        for t in range(n_kei):
            if syudan[t,0] == j:
                ...
                print(j,"の時間についかされた",syudan[t,1],"のIDの",moku_t,"目的地")
    ...
    print(j,"の時間についかされた",mokuteki,"目的地の集合星")
...
aaa = Path("hozon")
anim.save(aaa/'mati_siro1125_16.gif', writer='writer', fps=2)
